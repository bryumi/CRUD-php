...
class COOPERATIVA_DAO extends DB {
	...
	public function findAll(){
		$sql = 'SELECT * FROM COOPERATIVAS ORDER BY COO_NOME';
		$stmt = DB::prepare($sql);
		$stmt->execute();
		$arr = $stmt->fetchAll();
		foreach($arr as $chave => $valor){
	...
	public function inserir($cooperativa){
		$sql = "INSERT INTO COOPERATIVAS (coo_nome, coo_nome_cadastrante, coo_mail_contato, coo_tpc_id, coo_logradouro, coo_bairro, coo_cidade, coo_estado, coo_cep, coo_aat_id) VALUES (':coo_nome',':coo_nome_cadastrante',':coo_mail_contato',:coo_tpc_id,':coo_logradouro',':coo_bairro',':coo_cidade',':coo_estado',':coo_cep',':coo_aat_id')";
		$stmt = DB::prepare($sql);
		
		$stmt->bindParam(':coo_nome',$cooperativa->getCoo_nome()); 
		$stmt->bindParam(':coo_nome_cadastrante',$cooperativa->getCoo_nome_cadastrante()); 
		$stmt->bindParam(':coo_mail_contato',$cooperativa->getCoo_mail_contato());
		$stmt->bindParam(':coo_tpc_id',$cooperativa->getIdTipo());
		$stmt->bindParam(':coo_logradouro',$cooperativa->getCoo_logradouro()); 
		$stmt->bindParam(':coo_bairro',$cooperativa->getCoo_bairro()); 
		$stmt->bindParam(':coo_cidade',$cooperativa->getCoo_cidade()); 
		$stmt->bindParam(':coo_estado',$cooperativa->getCoo_estado()); 
		$stmt->bindParam(':coo_cep',$cooperativa->getCoo_cep()); 
		$stmt->bindParam(':coo_aat_id',$cooperativa->getIdArea()); 
		...
		/*if($stmt->execute())
			...
			return false;*/
		try{
		    echo $cooperativa->getCoo_dt_cadastro();
		    $stmt->execute();
